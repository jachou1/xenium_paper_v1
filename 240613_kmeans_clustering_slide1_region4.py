...
slide1_region4_tissue = concat_ad[concat_ad.obs['tissue'] == 'slide1_r4']
...
def generate_neighbor_graph(single_cell_matrix, distance_threshold=100):
    ...
    kdt = KDTree(single_cell_matrix.obsm['spatial'])

    # get the nearest 100 meighbors using kdTree
    distances_matrix, neighbor_indices = kdt.query(single_cell_matrix.obsm['spatial'], k=151)
...
distances_matrix, neighbor_graph, distances_mask = generate_neighbor_graph(slide1_region4_tissue)
...
positions = {i: tuple(coord) for i, coord in enumerate(slide1_region4_tissue.obsm['spatial'])}
...
plt.title("Spatial Network: Slide1 Region 4")
plt.gca().set_aspect('equal')
plt.gca().invert_yaxis()
plt.savefig('/Users/jacquelinechou/Downloads/240613_redo_s1_r4_spatial_network_90x90_200dpi.png');
...
# new_indices = np.arange(0, slide1_region4_tissue.obs.shape[0], 1).astype('str')
# s1_r4_df_reindexed = slide1_region4_tissue
# s1_r4_df_reindexed.obs = slide1_region4_tissue.obs.set_index(new_indices)

# Let's get the cell-type labels per cell using neighbor graph
subset_list = [slide1_region4_tissue.obs['celltype_tier1_merged'].iloc[neighbors].values for neighbors in filtered_neighbors]

celltype_labels = slide1_region4_tissue.obs['celltype_tier1_merged'].unique()
...
slide1_region4_tissue.obs['spatial_neighborhood_k_2'] = labels

# Let's plot what the 'spatial_neighborhood_k_2' looks like
plt.figure(figsize= (60, 60), dpi = 100)
plt.scatter(x=slide1_region4_tissue[slide1_region4_tissue.obs['spatial_neighborhood_k_2'] == 1].obsm['spatial'][:, 0],
            y=slide1_region4_tissue[slide1_region4_tissue.obs['spatial_neighborhood_k_2'] == 1].obsm['spatial'][:, 1],
            c = 'lightskyblue', s = 1)
plt.legend([''])
plt.gca().set_aspect('equal')
plt.gca().invert_yaxis()
plt.savefig('/Users/jacquelinechou/Downloads/updated_non-plasma_Bcells_slide1_region4_neighbor_k_2_spatial_plot.png')
...
np.save('/Users/jacquelinechou/Downloads/s1_r4_centroids5_kmeans5.npy', centroids5)

## So, the cell assignments to spatial domains is off when I re-run kmeans. I don't think the kmeans would be that different
# Let's generate the df that I should've saved.
...
# But let's 'match' up the spatial clusters: they are the closest clusters that were identified first
spatial_neighborhoods_df['Spatial_neighborhood'] = ['Spatial cluster 3', 'Spatial cluster 4', 'Spatial cluster 2',
                                                    'Spatial cluster 1', 'Spatial cluster 0']

spatial_neighborhoods_df.to_csv('/Users/jacquelinechou/Downloads/s1_r4/240729_revisited_slide1_region4_kmeans_5_df.csv')

# Stacked barplot
spatial_neighborhoods_df.sort_values(by= 'Spatial_neighborhood').plot(x = 'Spatial_neighborhood', kind = 'bar', stacked=True)
plt.legend(loc='upper right', bbox_to_anchor=(1.5, 0.5))
plt.savefig('/Users/jacquelinechou/Downloads/240729_s1_r4_celltype_composition_barplot_kmeans_5.png', bbox_inches='tight')

# Add these spatial neighborhood_k=2 to the anndata object
slide1_region4_tissue.obs['spatial_neighborhood_k_5'] = labels5

from matplotlib import colors as mcolors
slide1_region4_tissue.obs['spatial_x'] = slide1_region4_tissue.obsm['spatial'][:, 0]
slide1_region4_tissue.obs['spatial_y'] = slide1_region4_tissue.obsm['spatial'][:, 1]

plt.figure(figsize=(20, 30), dpi= 300)
for color, group in slide1_region4_tissue.obs.groupby(['spatial_neighborhood_k_5']):
    idx = color[0]
    # print(idx)
...
plt.savefig('/Users/jacquelinechou/Downloads/s1_r4_test_spatial_plot_k_5.png')
...
plt.legend(loc='upper right', bbox_to_anchor=(1.5, 0.5))
plt.savefig('/Users/jacquelinechou/Downloads/240613_s1_r4_celltype_composition_barplot_kmeans_5.png', bbox_inches='tight')
...
slide1_region4_tissue.obs['fib_spatial_group=2_niche'] = np.where((slide1_region4_tissue.obs['celltype_tier1_merged'] == 'Sublining fibroblasts') &
                                                   (slide1_region4_tissue.obs['spatial_neighborhood_k_5'] == 2), '1', '0')

# Let's run differential gene expression analysis using scanpy
sc.tl.rank_genes_groups(slide1_region4_tissue, groupby = 'fib_spatial_group=2_niche', method = 'wilcoxon', key_added = 'fib_sp=2_niche_degs')

sc.pl.rank_genes_groups_dotplot(
    slide1_region4_tissue, groupby="fib_spatial_group=2_niche", standard_scale="var", n_genes=15, key = 'fib_sp=2_niche_degs',
    save = 's1_r4_fibs_leiden_tb_spatial_group_degs_dotplot.png')
...
# Let's create a new column in the anndata wherein we create a boolean mask for whether sublining fibroblasts are within spatial neighborhood, k=2
slide1_region4_tissue.obs['tcell_spatial_group=2_niche'] = np.where((slide1_region4_tissue.obs['celltype_tier1_merged'] == 'T-cells') &
                                                   (slide1_region4_tissue.obs['spatial_neighborhood_k_5'] == 2), '1', '0')

# Let's run differential gene expression analysis using scanpy
sc.tl.rank_genes_groups(slide1_region4_tissue, groupby = 'tcell_spatial_group=2_niche', method = 'wilcoxon', key_added = 'tcell_sp=2_niche_degs')

sc.pl.rank_genes_groups_dotplot(
    slide1_region4_tissue, groupby="tcell_spatial_group=2_niche", standard_scale="var", n_genes=25, key = 'tcell_sp=2_niche_degs',
...
slide1_region4_tissue.obs['nonplasma_bcell_spatial_group=2_niche'] = np.where((slide1_region4_tissue.obs['celltype_tier1_merged'] == 'Non-plasma B-cells') &
                                                   (slide1_region4_tissue.obs['spatial_neighborhood_k_5'] == 2), '1', '0')
# Let's run differential gene expression analysis using scanpy
sc.tl.rank_genes_groups(slide1_region4_tissue, groupby = 'nonplasma_bcell_spatial_group=2_niche', method = 'wilcoxon', key_added = 'nonplasma_bcell_sp=2_niche_degs')

sc.pl.rank_genes_groups_dotplot(
    slide1_region4_tissue, groupby="nonplasma_bcell_spatial_group=2_niche", standard_scale="var", n_genes=15, key = 'nonplasma_bcell_sp=2_niche_degs',
...
df = sc.get.rank_genes_groups_df(slide1_region4_tissue, group='1', key='tcell_sp=2_niche_degs')
...
for gene in np.arange(len(top_tcells_degs_spatial_k2)):
    ...
    plt.figure(figsize=(40 / 2, 20 / 2), dpi=300)
    plt.scatter(x=slide1_region4_tissue[slide1_region4_tissue.obs['spatial_neighborhood_k_5'] == 2].obsm['spatial'][:, 0],
                y=slide1_region4_tissue[slide1_region4_tissue.obs['spatial_neighborhood_k_5'] == 2].obsm['spatial'][:, 1],
                c='lightskyblue', s=1)
    plt.scatter(x=np.array(slide1_region4_tissue[slide1_region4_tissue.obs['celltype_tier1_merged'] == 'T-cells'].obsm['spatial'][:, 0]),
                y=np.array(slide1_region4_tissue[slide1_region4_tissue.obs['celltype_tier1_merged'] == 'T-cells'].obsm['spatial'][:, 1]),
                s=1, cmap= 'inferno_r', alpha = 0.5,
                c=np.array(slide1_region4_tissue.layers['counts'][(slide1_region4_tissue.obs['celltype_tier1_merged'] == 'T-cells'),
                slide1_region4_tissue[slide1_region4_tissue.obs['celltype_tier1_merged'] == 'T-cells'].var_names == gene_of_interest]))
    plt.gca().invert_yaxis()
    plt.gca().set_aspect('equal')
    plt.title(f'{gene_of_interest} expression in T-cells only  \n spatial_neighborhood = 2 (S2_R4)')
    plt.colorbar(shrink=0.5)
    plt.savefig(f'/Users/jacquelinechou/Downloads/240612_s2_r4_{gene_of_interest}_expression_all_tcells.png')

df_bcells = sc.get.rank_genes_groups_df(slide1_region4_tissue, group='1', key='nonplasma_bcell_sp=2_niche_degs')
...
for gene in np.arange(len(top_nonplasma_cells_degs_spatial_k2)):
    ...
    plt.scatter(x=slide1_region4_tissue[slide1_region4_tissue.obs['spatial_neighborhood_k_5'] == 2].obsm['spatial'][:, 0],
                y=slide1_region4_tissue[slide1_region4_tissue.obs['spatial_neighborhood_k_5'] == 2].obsm['spatial'][:, 1],
                c='lightskyblue', s=1)
    plt.scatter(x=np.array(slide1_region4_tissue[slide1_region4_tissue.obs['celltype_tier1_merged'] == 'Non-plasma B-cells'].obsm['spatial'][:, 0]),
                y=np.array(slide1_region4_tissue[slide1_region4_tissue.obs['celltype_tier1_merged'] == 'Non-plasma B-cells'].obsm['spatial'][:, 1]),
                s=1, cmap= 'inferno_r', alpha = 0.5,
                c=np.array(slide1_region4_tissue.layers['counts'][(slide1_region4_tissue.obs['celltype_tier1_merged'] == 'Non-plasma B-cells'),
                slide1_region4_tissue[slide1_region4_tissue.obs['celltype_tier1_merged'] == 'Non-plasma B-cells'].var_names == gene_of_interest]))
    plt.gca().invert_yaxis()
    plt.gca().set_aspect('equal')
    plt.title(f'{gene_of_interest} expression in Non-plasma B-cells only  \n spatial_neighborhood = 2 (S1_R4)')
    plt.colorbar(shrink=0.5)
    plt.savefig(f'/Users/jacquelinechou/Downloads/240612_s1_r4_{gene_of_interest}_expression_all_non_plasma_bcells.png')

# Save h5ad file so far
slide1_region4_tissue.write('/Users/jacquelinechou/Downloads/240612_s1_r4_post_kmeans_tissue.h5ad')

# Let's run through a for loop for all 4 cell-types (Sublining fibroblasts, T-cells, and non-plasma B-cells)
celltypes_interest = ['Sublining fibroblasts', 'T-cells', 'Non-plasma B-cells', 'Myeloid cells']
tb_spatial_group = int(1)
for celltype in celltypes_interest:
    slide1_region4_tissue.obs[f'{celltype}_spatial_group={tb_spatial_group}_niche'] = np.where(
        (slide1_region4_tissue.obs['celltype_tier1_merged'] == celltype) &
        (slide1_region4_tissue.obs['spatial_neighborhood_k_5'] == tb_spatial_group), '1', '0')

    # Let's run differential gene expression analysis using scanpy
    sc.tl.rank_genes_groups(slide1_region4_tissue, groupby=f'{celltype}_spatial_group={tb_spatial_group}_niche', method='wilcoxon',
                            key_added=f'{celltype}_sp={tb_spatial_group}_niche_degs')

    sc.pl.rank_genes_groups_dotplot(
        slide1_region4_tissue, groupby=f'{celltype}_spatial_group={tb_spatial_group}_niche', standard_scale="var", n_genes=15,
        key = f'{celltype}_sp={tb_spatial_group}_niche_degs',
        save=f's1_r4_{celltype}_leiden_tb_spatial_group_degs_dotplot.png')

df_bcells = sc.get.rank_genes_groups_df(slide1_region4_tissue, group='1', key='Non-plasma B-cells_sp=1_niche_degs')
...
for gene in np.arange(len(top_nonplasma_cells_degs_spatial_k2)):
    ...
    print(gene_of_interest)
    plt.figure(figsize=(40 / 2, 20 / 2), dpi=300)
    plt.scatter(x=slide1_region4_tissue[slide1_region4_tissue.obs['spatial_neighborhood_k_5'] == tb_spatial_group].obsm['spatial'][:, 0],
                y=slide1_region4_tissue[slide1_region4_tissue.obs['spatial_neighborhood_k_5'] == tb_spatial_group].obsm['spatial'][:, 1],
                c='lightskyblue', s=1)
    plt.scatter(x=np.array(slide1_region4_tissue[slide1_region4_tissue.obs['celltype_tier1_merged'] == 'Non-plasma B-cells'].obsm['spatial'][:, 0]),
                y=np.array(slide1_region4_tissue[slide1_region4_tissue.obs['celltype_tier1_merged'] == 'Non-plasma B-cells'].obsm['spatial'][:, 1]),
                s=1, cmap= 'inferno_r', alpha = 0.5,
    ...
    plt.title(f'{gene_of_interest} expression in Non-plasma B-cells only  \n spatial_neighborhood = 2 (S1_R4)')
    plt.colorbar(shrink=0.5)
    plt.savefig(f'/Users/jacquelinechou/Downloads/240613_s1_r4_{gene_of_interest}_expression_all_non_plasma_bcells.png')

# I realized that I wasn't actually comparing b/w the same cell-type in one spatial niche vs. another spatial niche
# The following is the correct code:
tb_spatial_loc = slide1_region4_tissue.obsm['spatial'][slide1_region4_tissue.obs['spatial_neighborhood_k_5'] == 3, :]

# Rather than jst looking at sublining fibroblasts, let's find all proximal cells (w/i 30 microns)
all_cells = slide1_region4_tissue.obsm['spatial']
...
slide1_region4_tissue.obs['proximal_to_tb'] = 0
slide1_region4_tissue.obs.iloc[proximal_cells_tb_indices, slide1_region4_tissue.obs.columns.get_loc('proximal_to_tb')] = 1

plt.figure(figsize=(20 / 2, 40 / 2), dpi=100)
# filter adata to just the proximal cells
proximal_x_coords = slide1_region4_tissue[slide1_region4_tissue.obs['proximal_to_tb'] == 1].obsm['spatial'][:, 0]
proximal_y_coords = slide1_region4_tissue[slide1_region4_tissue.obs['proximal_to_tb'] == 1].obsm['spatial'][:, 1]
plt.scatter(x = proximal_x_coords, y = proximal_y_coords, s = 1, color = 'blue')
# Plot the spatial neighborhood, T/B aggregates
plt.scatter(x = tb_spatial_loc[:, 0], y = tb_spatial_loc[:, 1], s = 1, color = 'orange')
plt.gca().invert_yaxis()
plt.gca().set_aspect('equal')
plt.savefig('/Users/jacquelinechou/Downloads/s1_r4_tb_kdtree_30_microns.png')

slide1_region4_tissue.obs['proximal_to_tb'] = slide1_region4_tissue.obs['proximal_to_tb'].astype('str')
# Let's create a column with celltype names and whether it's proximal (1) or distal to TB spatial niche
slide1_region4_tissue.obs['celltype_tier1_proximal_tb'] = slide1_region4_tissue.obs['cell_type_tier1_fixed'].str.cat(slide1_region4_tissue.obs['proximal_to_tb'])

celltypes_interest = ['Myeloid cells', 'T-cells', 'Non-plasma B-cells', 'Lining fibroblasts', 'Sublining fibroblasts']

for celltype in celltypes_interest:
    sc.tl.rank_genes_groups(slide1_region4_tissue, groupby = 'celltype_tier1_proximal_tb', groups = [f'{celltype}1'],
                        reference = f'{celltype}0', method = 'wilcoxon', key_added = f'{celltype}_1_ref_0_degs')

    sc.pl.rank_genes_groups_dotplot(
        slide1_region4_tissue, groupby = 'celltype_tier1_proximal_tb', standard_scale="var", n_genes=15, key = f'{celltype}_1_ref_0_degs',
        save = f's1_r4_{celltype}_in_tb_niche_vs_out_dotplot.png')
    # Create mask for cell-types of interest for plotting [0 v. 1 per celltype]
    mask = slide1_region4_tissue.obs['celltype_tier1_proximal_tb'].isin([f'{celltype}1', f'{celltype}0'])
    top_5_genes = pd.DataFrame(slide1_region4_tissue.uns[f'{celltype}_1_ref_0_degs']['names']).head(5)
    genes_to_plot = top_5_genes.values.flatten()
    # Let's make the violin plots for each celltype:
    sc.pl.violin(slide1_region4_tissue[mask], keys = genes_to_plot, groupby = 'celltype_tier1_proximal_tb', save = f's1_r4_{celltype}_tb_1_0.png')

# Let's get the CSV file of DEGs:
for celltype in celltypes_interest:
    celltype_tb_degs = sc.get.rank_genes_groups_df(slide1_region4_tissue, key = f'{celltype}_1_ref_0_degs',
                                                 group = f'{celltype}1', pval_cutoff= 0.05)

    celltype_tb_degs.to_csv(f'/Users/jacquelinechou/Downloads/s1_r4_{celltype}_tb_degs.csv')

slide1_region4_tissue.write('/Users/jacquelinechou/Downloads/240701_slide1_region4_tb_kmeans.h5ad')
