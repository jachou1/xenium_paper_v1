...
plt.savefig('/Users/jacquelinechou/Downloads/240611_redo_s2_r4_spatial_network_90x90_300dpi.png');
...
celltype_labels = slide2_region4_tissue.obs['celltype_tier1_merged'].unique()
...
plt.gca().invert_yaxis()
plt.savefig('/Users/jacquelinechou/Downloads/updated_non-plasma_Bcells_slide2_region4_neighbor_k_2_spatial_plot.png')
...
np.save('/Users/jacquelinechou/Downloads/s2_r4_centroids5_kmeans5.npy', centroids5)
...
plt.savefig('/Users/jacquelinechou/Downloads/s2_r4_test_spatial_plot_k_5.png')
...
plt.savefig('/Users/jacquelinechou/Downloads/s2_r4_celltype_composition_barplot_kmeans_5.png', bbox_inches='tight')
...
slide2_region4_tissue.obs['fib_spatial_group=2_niche'] = np.where((slide2_region4_tissue.obs['celltype_tier1_merged'] == 'Sublining fibroblasts') &
                                                   (slide2_region4_tissue.obs['spatial_neighborhood_k_5'] == 2), '1', '0')

# Let's run differential gene expression analysis using scanpy
sc.tl.rank_genes_groups(slide2_region4_tissue, groupby = 'fib_spatial_group=2_niche', method = 'wilcoxon', key_added = 'fib_sp=2_niche_degs')

sc.pl.rank_genes_groups_dotplot(
    slide2_region4_tissue, groupby="fib_spatial_group=2_niche", standard_scale="var", n_genes=15, key = 'fib_sp=2_niche_degs',
    save = 's2_r4_fibs_leiden_tb_spatial_group_degs_dotplot.png')
...
# Let's create a new column in the anndata wherein we create a boolean mask for whether sublining fibroblasts are within spatial neighborhood, k=2
slide2_region4_tissue.obs['tcell_spatial_group=2_niche'] = np.where((slide2_region4_tissue.obs['celltype_tier1_merged'] == 'T-cells') &
                                                   (slide2_region4_tissue.obs['spatial_neighborhood_k_5'] == 2), '1', '0')

# Let's run differential gene expression analysis using scanpy
sc.tl.rank_genes_groups(slide2_region4_tissue, groupby = 'tcell_spatial_group=2_niche', method = 'wilcoxon', key_added = 'tcell_sp=2_niche_degs')

sc.pl.rank_genes_groups_dotplot(
    slide2_region4_tissue, groupby="tcell_spatial_group=2_niche", standard_scale="var", n_genes=25, key = 'tcell_sp=2_niche_degs',
...
slide2_region4_tissue.obs['nonplasma_bcell_spatial_group=2_niche'] = np.where((slide2_region4_tissue.obs['celltype_tier1_merged'] == 'Non-plasma B-cells') &
                                                   (slide2_region4_tissue.obs['spatial_neighborhood_k_5'] == 2), '1', '0')
# Let's run differential gene expression analysis using scanpy
sc.tl.rank_genes_groups(slide2_region4_tissue, groupby = 'nonplasma_bcell_spatial_group=2_niche', method = 'wilcoxon', key_added = 'nonplasma_bcell_sp=2_niche_degs')

sc.pl.rank_genes_groups_dotplot(
    slide2_region4_tissue, groupby="nonplasma_bcell_spatial_group=2_niche", standard_scale="var", n_genes=15, key = 'nonplasma_bcell_sp=2_niche_degs',
...
df = sc.get.rank_genes_groups_df(slide2_region4_tissue, group='1', key='tcell_sp=2_niche_degs')
...
for gene in np.arange(len(top_tcells_degs_spatial_k2)):
    ...
    plt.figure(figsize=(40 / 2, 20 / 2), dpi=300)
    plt.scatter(x=slide2_region4_tissue[slide2_region4_tissue.obs['spatial_neighborhood_k_5'] == 2].obsm['spatial'][:, 0],
                y=slide2_region4_tissue[slide2_region4_tissue.obs['spatial_neighborhood_k_5'] == 2].obsm['spatial'][:, 1],
                c='lightskyblue', s=1)
    plt.scatter(x=np.array(slide2_region4_tissue[slide2_region4_tissue.obs['celltype_tier1_merged'] == 'T-cells'].obsm['spatial'][:, 0]),
                y=np.array(slide2_region4_tissue[slide2_region4_tissue.obs['celltype_tier1_merged'] == 'T-cells'].obsm['spatial'][:, 1]),
                s=1, cmap= 'inferno_r', alpha = 0.5,
                c=np.array(slide2_region4_tissue.layers['counts'][(slide2_region4_tissue.obs['celltype_tier1_merged'] == 'T-cells'),
                slide2_region4_tissue[slide2_region4_tissue.obs['celltype_tier1_merged'] == 'T-cells'].var_names == gene_of_interest]))
    plt.gca().invert_yaxis()
    plt.gca().set_aspect('equal')
...
df_bcells = sc.get.rank_genes_groups_df(slide2_region4_tissue, group='1', key='nonplasma_bcell_sp=2_niche_degs')
...
for gene in np.arange(len(top_nonplasma_cells_degs_spatial_k2)):
    ...
    plt.scatter(x=slide2_region4_tissue[slide2_region4_tissue.obs['spatial_neighborhood_k_5'] == 2].obsm['spatial'][:, 0],
                y=slide2_region4_tissue[slide2_region4_tissue.obs['spatial_neighborhood_k_5'] == 2].obsm['spatial'][:, 1],
                c='lightskyblue', s=1)
    plt.scatter(x=np.array(slide2_region4_tissue[slide2_region4_tissue.obs['celltype_tier1_merged'] == 'Non-plasma B-cells'].obsm['spatial'][:, 0]),
                y=np.array(slide2_region4_tissue[slide2_region4_tissue.obs['celltype_tier1_merged'] == 'Non-plasma B-cells'].obsm['spatial'][:, 1]),
                s=1, cmap= 'inferno_r', alpha = 0.5,
                c=np.array(slide2_region4_tissue.layers['counts'][(slide2_region4_tissue.obs['celltype_tier1_merged'] == 'Non-plasma B-cells'),
                slide2_region4_tissue[slide2_region4_tissue.obs['celltype_tier1_merged'] == 'Non-plasma B-cells'].var_names == gene_of_interest]))
    plt.gca().invert_yaxis()
    plt.gca().set_aspect('equal')
    plt.title(f'{gene_of_interest} expression in Non-plasma B-cells only  \n spatial_neighborhood = 2 (S2_R4)')
    plt.colorbar(shrink=0.5)
    plt.savefig(f'/Users/jacquelinechou/Downloads/240612_s2_r4_{gene_of_interest}_expression_all_non_plasma_bcells.png')

# Save h5ad file so far
slide2_region4_tissue.write('/Users/jacquelinechou/Downloads/240612_s2_r4_post_kmeans_tissue.h5ad')

# Let's do the same for Slide1_Region4