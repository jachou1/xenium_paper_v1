...
# Let's just look at slide1_region5 (T/B aggregate heavy, good to test out spatial niche test)
slide1_region5_tissue = concat_ad[concat_ad.obs['tissue'] == 'slide1_r5']
...
distances_matrix, neighbor_graph, distances_mask = generate_neighbor_graph(slide1_region5_tissue)
...
positions = {i: tuple(coord) for i, coord in enumerate(slide1_region5_tissue.obsm['spatial'])}
...
plt.savefig('/Users/jacquelinechou/Downloads/s1_r5/240619_s1_r5_spatial_network_90x90_200dpi.png');
...
subset_list = [slide1_region5_tissue.obs['celltype_tier1_merged'].iloc[neighbors].values for neighbors in filtered_neighbors]

celltype_labels = slide1_region5_tissue.obs['celltype_tier1_merged'].unique()
...
np.save('/Users/jacquelinechou/Downloads/s1_r5_centroids5_kmeans5.npy', centroids5)

# Add these spatial neighborhood_k=2 to the anndata object
slide1_region5_tissue.obs['spatial_neighborhood_k_5'] = labels5

from matplotlib import colors as mcolors
slide1_region5_tissue.obs['spatial_x'] = slide1_region5_tissue.obsm['spatial'][:, 0]
slide1_region5_tissue.obs['spatial_y'] = slide1_region5_tissue.obsm['spatial'][:, 1]

plt.figure(figsize=(20, 30), dpi= 300)
for color, group in slide1_region5_tissue.obs.groupby(['spatial_neighborhood_k_5']):
    idx = color[0]
    # print(idx)
...
plt.savefig('/Users/jacquelinechou/Downloads/s1_r5/s1_r5_test_spatial_plot_k_5.png')
...
spatial_neighborhoods_df.to_csv('/Users/jacquelinechou/Downloads/s1_r5/slide1_region5_kmeans_5_df.csv')
# Stacked barplot
...
plt.savefig('/Users/jacquelinechou/Downloads/s1_r5/240619_s1_r5_celltype_composition_barplot_kmeans_5.png', bbox_inches='tight')
...
slide1_region5_tissue.obs['fib_spatial_group=1_niche'] = np.where((slide1_region5_tissue.obs['celltype_tier1_merged'] == 'Sublining fibroblasts') &
                                                   (slide1_region5_tissue.obs['spatial_neighborhood_k_5'] == 1), '1', '0')

# Let's run differential gene expression analysis using scanpy
sc.tl.rank_genes_groups(slide1_region5_tissue, groupby = 'fib_spatial_group=1_niche', method = 'wilcoxon', key_added = 'fib_sp=1_niche_degs')

sc.pl.rank_genes_groups_dotplot(
    slide1_region5_tissue, groupby="fib_spatial_group=1_niche", standard_scale="var", n_genes=15, key = 'fib_sp=1_niche_degs',
...
for gene in np.arange(len(top_fibs_degs_spatial_k2)):
    ...
    plt.figure(figsize=(40 / 2, 20 / 2), dpi=300)
    plt.scatter(x=slide1_region5_tissue[slide1_region5_tissue.obs['spatial_neighborhood_k_5'] == tb_spatial_group].obsm['spatial'][:, 0],
                y=slide1_region5_tissue[slide1_region5_tissue.obs['spatial_neighborhood_k_5'] == tb_spatial_group].obsm['spatial'][:, 1],
                c='lightskyblue', s=1)
    plt.scatter(x=np.array(slide1_region5_tissue[slide1_region5_tissue.obs['celltype_tier1_merged'] == 'Sublining fibroblasts'].obsm['spatial'][:, 0]),
                y=np.array(slide1_region5_tissue[slide1_region5_tissue.obs['celltype_tier1_merged'] == 'Sublining fibroblasts'].obsm['spatial'][:, 1]),
                s=1, cmap= 'inferno_r',
                c=np.array(slide1_region5_tissue.layers['counts'][(slide1_region5_tissue.obs['celltype_tier1_merged'] == 'Sublining fibroblasts'),
                slide1_region5_tissue[slide1_region5_tissue.obs['celltype_tier1_merged'] == 'Sublining fibroblasts'].var_names == gene_of_interest]))
    plt.gca().invert_yaxis()
    plt.gca().set_aspect('equal')
...
leiden_condition = slide1_region5_tissue.obs['celltype_tier1_merged'] == 'Sublining fibroblasts'
genes_condition = slide1_region5_tissue.var_names.isin(top_fibs_15_degs_spatial_k2)
top15_gene_count = slide1_region5_tissue.layers['counts'][leiden_condition][:, genes_condition].sum(axis = 1)
# Extract the spatial data for the specific condition
spatial_data = slide1_region5_tissue[leiden_condition].obsm['spatial']
...
plt.scatter(x=slide1_region5_tissue[slide1_region5_tissue.obs['spatial_neighborhood_k_5'] == tb_spatial_group].obsm['spatial'][:, 0],
            y=slide1_region5_tissue[slide1_region5_tissue.obs['spatial_neighborhood_k_5'] == tb_spatial_group].obsm['spatial'][:, 1],
            c='lightskyblue', s=1)
plt.scatter(x=np.array(slide1_region5_tissue[leiden_condition].obsm['spatial'][:, 0]),
            y=np.array(slide1_region5_tissue[leiden_condition].obsm['spatial'][:, 1]),
...
# Let's create a new column in the anndata wherein we create a boolean mask for whether sublining fibroblasts are within spatial neighborhood, k=2
slide1_region5_tissue.obs['tcell_spatial_group=2_niche'] = np.where((slide1_region5_tissue.obs['celltype_tier1_merged'] == 'T-cells') &
                                                   (slide1_region5_tissue.obs['spatial_neighborhood_k_5'] == 2), '1', '0')

# Let's run differential gene expression analysis using scanpy
sc.tl.rank_genes_groups(slide1_region5_tissue, groupby = 'tcell_spatial_group=2_niche', method = 'wilcoxon', key_added = 'tcell_sp=2_niche_degs')

sc.pl.rank_genes_groups_dotplot(
    slide1_region5_tissue, groupby="tcell_spatial_group=2_niche", standard_scale="var", n_genes=25, key = 'tcell_sp=2_niche_degs',
...
slide1_region5_tissue.obs['nonplasma_bcell_spatial_group=2_niche'] = np.where((slide1_region5_tissue.obs['celltype_tier1_merged'] == 'Non-plasma B-cells') &
                                                   (slide1_region5_tissue.obs['spatial_neighborhood_k_5'] == 2), '1', '0')
# Let's run differential gene expression analysis using scanpy
sc.tl.rank_genes_groups(slide1_region5_tissue, groupby = 'nonplasma_bcell_spatial_group=2_niche', method = 'wilcoxon', key_added = 'nonplasma_bcell_sp=2_niche_degs')

sc.pl.rank_genes_groups_dotplot(
    slide1_region5_tissue, groupby="nonplasma_bcell_spatial_group=2_niche", standard_scale="var", n_genes=15, key = 'nonplasma_bcell_sp=2_niche_degs',
...
df = sc.get.rank_genes_groups_df(slide1_region5_tissue, group='1', key='tcell_sp=2_niche_degs')
...
for gene in np.arange(len(top_tcells_degs_spatial_k2)):
    ...
    plt.figure(figsize=(40 / 2, 20 / 2), dpi=300)
    plt.scatter(x=slide1_region5_tissue[slide1_region5_tissue.obs['spatial_neighborhood_k_5'] == 2].obsm['spatial'][:, 0],
                y=slide1_region5_tissue[slide1_region5_tissue.obs['spatial_neighborhood_k_5'] == 2].obsm['spatial'][:, 1],
                c='lightskyblue', s=1)
    plt.scatter(x=np.array(slide1_region5_tissue[slide1_region5_tissue.obs['celltype_tier1_merged'] == 'T-cells'].obsm['spatial'][:, 0]),
                y=np.array(slide1_region5_tissue[slide1_region5_tissue.obs['celltype_tier1_merged'] == 'T-cells'].obsm['spatial'][:, 1]),
                s=1, cmap= 'inferno_r', alpha = 0.5,
                c=np.array(slide1_region5_tissue.layers['counts'][(slide1_region5_tissue.obs['celltype_tier1_merged'] == 'T-cells'),
                slide1_region5_tissue[slide1_region5_tissue.obs['celltype_tier1_merged'] == 'T-cells'].var_names == gene_of_interest]))
    plt.gca().invert_yaxis()
    plt.gca().set_aspect('equal')
...
df_bcells = sc.get.rank_genes_groups_df(slide1_region5_tissue, group='1', key='nonplasma_bcell_sp=2_niche_degs')
...
for gene in np.arange(len(top_nonplasma_cells_degs_spatial_k2)):
    ...
    plt.scatter(x=slide1_region5_tissue[slide1_region5_tissue.obs['spatial_neighborhood_k_5'] == 2].obsm['spatial'][:, 0],
                y=slide1_region5_tissue[slide1_region5_tissue.obs['spatial_neighborhood_k_5'] == 2].obsm['spatial'][:, 1],
                c='lightskyblue', s=1)
    plt.scatter(x=np.array(slide1_region5_tissue[slide1_region5_tissue.obs['celltype_tier1_merged'] == 'Non-plasma B-cells'].obsm['spatial'][:, 0]),
                y=np.array(slide1_region5_tissue[slide1_region5_tissue.obs['celltype_tier1_merged'] == 'Non-plasma B-cells'].obsm['spatial'][:, 1]),
                s=1, cmap= 'inferno_r', alpha = 0.5,
                c=np.array(slide1_region5_tissue.layers['counts'][(slide1_region5_tissue.obs['celltype_tier1_merged'] == 'Non-plasma B-cells'),
                slide1_region5_tissue[slide1_region5_tissue.obs['celltype_tier1_merged'] == 'Non-plasma B-cells'].var_names == gene_of_interest]))
    plt.gca().invert_yaxis()
    plt.gca().set_aspect('equal')
...
tb_spatial_group = int(4)
tissue = 'slide1_region5'
for celltype in celltypes_interest:
    slide1_region5_tissue.obs[f'{celltype}_spatial_group={tb_spatial_group}_niche'] = np.where(
        (slide1_region5_tissue.obs['celltype_tier1_merged'] == celltype) &
        (slide1_region5_tissue.obs['spatial_neighborhood_k_5'] == tb_spatial_group), '1', '0')

    # Let's run differential gene expression analysis using scanpy
    sc.tl.rank_genes_groups(slide1_region5_tissue, groupby=f'{celltype}_spatial_group={tb_spatial_group}_niche', method='wilcoxon',
                            key_added=f'{celltype}_sp={tb_spatial_group}_niche_degs')

    sc.pl.rank_genes_groups_dotplot(
        slide1_region5_tissue, groupby=f'{celltype}_spatial_group={tb_spatial_group}_niche', standard_scale="var", n_genes=15,
...
slide1_region5_tissue.write('/Users/jacquelinechou/Downloads/s1_r5/240618_s1_r5_post_kmeans_tissue.h5ad')

df_bcells = sc.get.rank_genes_groups_df(slide1_region5_tissue, group='1', key='Non-plasma B-cells_sp=1_niche_degs')
...
# DGE analysis on TB-localized cells:
tb_spatial_loc = slide1_region5_tissue.obsm['spatial'][slide1_region5_tissue.obs['spatial_neighborhood_k_5'] == 4, :]

# Rather than jst looking at sublining fibroblasts, let's find all proximal cells (w/i 30 microns)
all_cells = slide1_region5_tissue.obsm['spatial']
...
slide1_region5_tissue.obs['proximal_to_tb'] = 0
slide1_region5_tissue.obs.iloc[proximal_cells_tb_indices, slide1_region5_tissue.obs.columns.get_loc('proximal_to_tb')] = 1

plt.figure(figsize=(20 / 2, 40 / 2), dpi=100)
# filter adata to just the proximal cells
proximal_x_coords = slide1_region5_tissue[slide1_region5_tissue.obs['proximal_to_tb'] == 1].obsm['spatial'][:, 0]
proximal_y_coords = slide1_region5_tissue[slide1_region5_tissue.obs['proximal_to_tb'] == 1].obsm['spatial'][:, 1]
...
plt.gca().invert_yaxis()
plt.gca().set_aspect('equal')
plt.savefig('/Users/jacquelinechou/Downloads/s1_r5_tb_kdtree_30_microns.png')

slide1_region5_tissue.obs['proximal_to_tb'] = slide1_region5_tissue.obs['proximal_to_tb'].astype('str')
# Let's create a column with celltype names and whether it's proximal (1) or distal to TB spatial niche
slide1_region5_tissue.obs['celltype_tier1_proximal_tb'] = slide1_region5_tissue.obs['celltype_tier1_merged'].str.cat(slide1_region5_tissue.obs['proximal_to_tb'])

celltypes_interest = ['Myeloid cells', 'T-cells', 'Non-plasma B-cells', 'Lining fibroblasts', 'Sublining fibroblasts']

for celltype in celltypes_interest:
    sc.tl.rank_genes_groups(slide1_region5_tissue, groupby = 'celltype_tier1_proximal_tb', groups = [f'{celltype}1'],
                        reference = f'{celltype}0', method = 'wilcoxon', key_added = f'{celltype}_1_ref_0_degs')

    sc.pl.rank_genes_groups_dotplot(
        slide1_region5_tissue, groupby = 'celltype_tier1_proximal_tb', standard_scale="var", n_genes=15, key = f'{celltype}_1_ref_0_degs',
        save = f's1_r5_{celltype}_in_tb_niche_vs_out_dotplot.png')
    # Create mask for cell-types of interest for plotting [0 v. 1 per celltype]
    mask = slide1_region5_tissue.obs['celltype_tier1_proximal_tb'].isin([f'{celltype}1', f'{celltype}0'])
    top_5_genes = pd.DataFrame(slide1_region5_tissue.uns[f'{celltype}_1_ref_0_degs']['names']).head(5)
    genes_to_plot = top_5_genes.values.flatten()
    # Let's make the violin plots for each celltype:
    sc.pl.violin(slide1_region5_tissue[mask], keys = genes_to_plot, groupby = 'celltype_tier1_proximal_tb', save = f's1_r5_{celltype}_tb_1_0.png')

# Let's get the CSV file of DEGs:
for celltype in celltypes_interest:
    celltype_tb_degs = sc.get.rank_genes_groups_df(slide1_region5_tissue, key = f'{celltype}_1_ref_0_degs',
                                                 group = f'{celltype}1', pval_cutoff= 0.05)

    celltype_tb_degs.to_csv(f'/Users/jacquelinechou/Downloads/s1_r5_{celltype}_tb_degs.csv')

slide1_region5_tissue.write('/Users/jacquelinechou/Downloads/240701_slide1_region5_tb_kmeans.h5ad')

# Let's make violin plots of normalized, log-transformed gene expression of CXCR4, CCR7, CXCL13, and IL7R
sc.pl.violin(slide1_region5_tissue[slide1_region5_tissue.obs['celltype_tier1_proximal_tb'].isin(['T-cells1', 'T-cells0'])],
             keys = ['CXCR4', 'CCR7'], groupby= 'celltype_tier1_proximal_tb', save = 's1_r5_tcell_tb_agg_cxcr4_ccr7_violin_plot.png')

sc.pl.violin(slide1_region5_tissue[slide1_region5_tissue.obs['celltype_tier1_proximal_tb'].isin(['T-cells1', 'T-cells0'])],
             keys = ['CXCL13', 'IL7R'], groupby= 'celltype_tier1_proximal_tb', save = 's1_r5_tcell_tb_agg_cxcl13_il7r_violin_plot.png')

# CCL19, CCR4, and CXCR5 were marked as upregulated in these TB aggregates
sc.pl.violin(slide1_region5_tissue[slide1_region5_tissue.obs['celltype_tier1_proximal_tb'].isin(['T-cells1', 'T-cells0'])],
             keys = ['CCL19', 'CCR4', 'CXCR5'], groupby = 'celltype_tier1_proximal_tb', save = 's1_r5_tcell_tb_agg_ccl19_ccr4_cxcr5_violin.png')