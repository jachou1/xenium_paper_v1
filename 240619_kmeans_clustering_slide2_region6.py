...
# Let's just look at slide2_region6 (T/B aggregate heavy, good to test out spatial niche test)
slide2_region6_tissue = concat_ad[concat_ad.obs['tissue'] == 'slide2_r6']
...
distances_matrix, neighbor_graph, distances_mask = generate_neighbor_graph(slide2_region6_tissue)
...
positions = {i: tuple(coord) for i, coord in enumerate(slide2_region6_tissue.obsm['spatial'])}
...
plt.savefig('/Users/jacquelinechou/Downloads/s2_r6/240619_s2_r6_spatial_network_90x90_200dpi.png');
...
subset_list = [slide2_region6_tissue.obs['celltype_tier1_merged'].iloc[neighbors].values for neighbors in filtered_neighbors]

celltype_labels = slide2_region6_tissue.obs['celltype_tier1_merged'].unique()
...
# slide2_region6_tissue.obs['spatial_neighborhood_k_2'] = labels

# Let's plot the spatial groups identified via k-means clustering
# plt.figure(figsize=(20, 30), dpi= 300)
# for color, group in slide2_region6_tissue.obs.groupby(['spatial_neighborhood_k_2']):
#     idx = color[0]
#     # print(idx)
...
np.save('/Users/jacquelinechou/Downloads/s2_r6/s2_r6_centroids5_kmeans5.npy', centroids5)

# Add these spatial neighborhood_k=2 to the anndata object
slide2_region6_tissue.obs['spatial_neighborhood_k_5'] = labels5

from matplotlib import colors as mcolors
slide2_region6_tissue.obs['spatial_x'] = slide2_region6_tissue.obsm['spatial'][:, 0]
slide2_region6_tissue.obs['spatial_y'] = slide2_region6_tissue.obsm['spatial'][:, 1]

plt.figure(figsize=(20, 30), dpi= 300)
for color, group in slide2_region6_tissue.obs.groupby(['spatial_neighborhood_k_5']):
    idx = color[0]
    # print(idx)
    # let's subset the adata to these neighborhoods
    plt.scatter(x = group['spatial_x'], y = group['spatial_y'], s= 1, c = list(mcolors.TABLEAU_COLORS)[idx], alpha = 0.5,
...
# legend.legendHandles[0]._legmarker.set_markersize(10)
# legend.legendHandles[0]._legmarker.set_alpha(1)
plt.savefig('/Users/jacquelinechou/Downloads/s2_r6/s2_r6_test_spatial_plot_k_5.png')
...
spatial_neighborhoods_df.to_csv('/Users/jacquelinechou/Downloads/s2_r6/slide2_region6_kmeans_5_df.csv')
# Stacked barplot
...
plt.savefig('/Users/jacquelinechou/Downloads/s2_r6/240619_s2_r6_celltype_composition_barplot_kmeans_5.png', bbox_inches='tight')
...
# slide2_region6_tissue.obs['fib_spatial_group=1_niche'] = np.where((slide2_region6_tissue.obs['celltype_tier1_merged'] == 'Sublining fibroblasts') &
#                                                    (slide2_region6_tissue.obs['spatial_neighborhood_k_5'] == 1), '1', '0')

# Let's run differential gene expression analysis using scanpy
# sc.tl.rank_genes_groups(slide2_region6_tissue, groupby = 'fib_spatial_group=1_niche', method = 'wilcoxon', key_added = 'fib_sp=1_niche_degs')
#
# sc.pl.rank_genes_groups_dotplot(
#     slide2_region6_tissue, groupby="fib_spatial_group=1_niche", standard_scale="var", n_genes=15, key = 'fib_sp=1_niche_degs',
...
slide2_region6_tissue.write('/Users/jacquelinechou/Downloads/s2_r6/240619_s2_r6_post_kmeans_tissue.h5ad')
...
for gene in np.arange(len(top_fibs_degs_spatial_k2)):
    ...
    plt.figure(figsize=(40 / 2, 20 / 2), dpi=300)
    plt.scatter(x=slide2_region6_tissue[slide2_region6_tissue.obs['spatial_neighborhood_k_5'] == tb_spatial_group].obsm['spatial'][:, 0],
                y=slide2_region6_tissue[slide2_region6_tissue.obs['spatial_neighborhood_k_5'] == tb_spatial_group].obsm['spatial'][:, 1],
                c='lightskyblue', s=1)
    plt.scatter(x=np.array(slide2_region6_tissue[slide2_region6_tissue.obs['celltype_tier1_merged'] == 'Sublining fibroblasts'].obsm['spatial'][:, 0]),
                y=np.array(slide2_region6_tissue[slide2_region6_tissue.obs['celltype_tier1_merged'] == 'Sublining fibroblasts'].obsm['spatial'][:, 1]),
                s=1, cmap= 'inferno_r',
                c=np.array(slide2_region6_tissue.layers['counts'][(slide2_region6_tissue.obs['celltype_tier1_merged'] == 'Sublining fibroblasts'),
                slide2_region6_tissue[slide2_region6_tissue.obs['celltype_tier1_merged'] == 'Sublining fibroblasts'].var_names == gene_of_interest]))
    plt.gca().invert_yaxis()
    plt.gca().set_aspect('equal')
...
leiden_condition = slide2_region6_tissue.obs['celltype_tier1_merged'] == 'Sublining fibroblasts'
genes_condition = slide2_region6_tissue.var_names.isin(top_fibs_15_degs_spatial_k2)
top15_gene_count = slide2_region6_tissue.layers['counts'][leiden_condition][:, genes_condition].sum(axis = 1)
# Extract the spatial data for the specific condition
spatial_data = slide2_region6_tissue[leiden_condition].obsm['spatial']
...
plt.scatter(x=slide2_region6_tissue[slide2_region6_tissue.obs['spatial_neighborhood_k_5'] == tb_spatial_group].obsm['spatial'][:, 0],
            y=slide2_region6_tissue[slide2_region6_tissue.obs['spatial_neighborhood_k_5'] == tb_spatial_group].obsm['spatial'][:, 1],
            c='lightskyblue', s=1)
plt.scatter(x=np.array(slide2_region6_tissue[leiden_condition].obsm['spatial'][:, 0]),
            y=np.array(slide2_region6_tissue[leiden_condition].obsm['spatial'][:, 1]),
...
# # Let's create a new column in the anndata wherein we create a boolean mask for whether sublining fibroblasts are within spatial neighborhood, k=2
# slide2_region6_tissue.obs['tcell_spatial_group=2_niche'] = np.where((slide2_region6_tissue.obs['celltype_tier1_merged'] == 'T-cells') &
#                                                    (slide2_region6_tissue.obs['spatial_neighborhood_k_5'] == 2), '1', '0')
#
# # Let's run differential gene expression analysis using scanpy
# sc.tl.rank_genes_groups(slide2_region6_tissue, groupby = 'tcell_spatial_group=2_niche', method = 'wilcoxon', key_added = 'tcell_sp=2_niche_degs')
#
# sc.pl.rank_genes_groups_dotplot(
#     slide2_region6_tissue, groupby="tcell_spatial_group=2_niche", standard_scale="var", n_genes=25, key = 'tcell_sp=2_niche_degs',
...
# slide2_region6_tissue.obs['nonplasma_bcell_spatial_group=2_niche'] = np.where((slide2_region6_tissue.obs['celltype_tier1_merged'] == 'Non-plasma B-cells') &
#                                                    (slide2_region6_tissue.obs['spatial_neighborhood_k_5'] == 2), '1', '0')
# # Let's run differential gene expression analysis using scanpy
# sc.tl.rank_genes_groups(slide2_region6_tissue, groupby = 'nonplasma_bcell_spatial_group=2_niche', method = 'wilcoxon', key_added = 'nonplasma_bcell_sp=2_niche_degs')
#
# sc.pl.rank_genes_groups_dotplot(
#     slide2_region6_tissue, groupby="nonplasma_bcell_spatial_group=2_niche", standard_scale="var", n_genes=15, key = 'nonplasma_bcell_sp=2_niche_degs',
...
# df = sc.get.rank_genes_groups_df(slide2_region6_tissue, group='1', key='tcell_sp=2_niche_degs')
#
...
#     plt.figure(figsize=(40 / 2, 20 / 2), dpi=300)
#     plt.scatter(x=slide2_region6_tissue[slide2_region6_tissue.obs['spatial_neighborhood_k_5'] == 2].obsm['spatial'][:, 0],
#                 y=slide2_region6_tissue[slide2_region6_tissue.obs['spatial_neighborhood_k_5'] == 2].obsm['spatial'][:, 1],
#                 c='lightskyblue', s=1)
#     plt.scatter(x=np.array(slide2_region6_tissue[slide2_region6_tissue.obs['celltype_tier1_merged'] == 'T-cells'].obsm['spatial'][:, 0]),
#                 y=np.array(slide2_region6_tissue[slide2_region6_tissue.obs['celltype_tier1_merged'] == 'T-cells'].obsm['spatial'][:, 1]),
#                 s=1, cmap= 'inferno_r', alpha = 0.5,
#                 c=np.array(slide2_region6_tissue.layers['counts'][(slide2_region6_tissue.obs['celltype_tier1_merged'] == 'T-cells'),
#                 slide2_region6_tissue[slide2_region6_tissue.obs['celltype_tier1_merged'] == 'T-cells'].var_names == gene_of_interest]))
#     plt.gca().invert_yaxis()
#     plt.gca().set_aspect('equal')
...
#
# df_bcells = sc.get.rank_genes_groups_df(slide2_region6_tissue, group='1', key='nonplasma_bcell_sp=2_niche_degs')
#
...
#                 y=slide2_region6_tissue[slide2_region6_tissue.obs['spatial_neighborhood_k_5'] == 2].obsm['spatial'][:, 1],
#                 c='lightskyblue', s=1)
#     plt.scatter(x=np.array(slide2_region6_tissue[slide2_region6_tissue.obs['celltype_tier1_merged'] == 'Non-plasma B-cells'].obsm['spatial'][:, 0]),
#                 y=np.array(slide2_region6_tissue[slide2_region6_tissue.obs['celltype_tier1_merged'] == 'Non-plasma B-cells'].obsm['spatial'][:, 1]),
#                 s=1, cmap= 'inferno_r', alpha = 0.5,
#                 c=np.array(slide2_region6_tissue.layers['counts'][(slide2_region6_tissue.obs['celltype_tier1_merged'] == 'Non-plasma B-cells'),
#                 slide2_region6_tissue[slide2_region6_tissue.obs['celltype_tier1_merged'] == 'Non-plasma B-cells'].var_names == gene_of_interest]))
#     plt.gca().invert_yaxis()
#     plt.gca().set_aspect('equal')
...
tissue = 'slide2_region6'
for celltype in celltypes_interest:
    slide2_region6_tissue.obs[f'{celltype}_spatial_group={tb_spatial_group}_niche'] = np.where(
        (slide2_region6_tissue.obs['celltype_tier1_merged'] == celltype) &
        (slide2_region6_tissue.obs['spatial_neighborhood_k_5'] == tb_spatial_group), '1', '0')

    # Let's run differential gene expression analysis using scanpy
    sc.tl.rank_genes_groups(slide2_region6_tissue, groupby=f'{celltype}_spatial_group={tb_spatial_group}_niche', method='wilcoxon',
                            key_added=f'{celltype}_sp={tb_spatial_group}_niche_degs')

    sc.pl.rank_genes_groups_dotplot(
        slide2_region6_tissue, groupby=f'{celltype}_spatial_group={tb_spatial_group}_niche', standard_scale="var", n_genes=15,
...
# df_bcells = sc.get.rank_genes_groups_df(slide2_region6_tissue, group='1', key='Non-plasma B-cells_sp=1_niche_degs')
#
# top_nonplasma_cells_degs_spatial_k2 = list(df_bcells['names'][0:15])
# for gene in np.arange(len(top_nonplasma_cells_degs_spatial_k2)):
#     gene_of_interest = top_nonplasma_cells_degs_spatial_k2[gene]
